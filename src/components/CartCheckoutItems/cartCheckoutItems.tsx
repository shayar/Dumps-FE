...
export default function CartCheckoutItems({
  ...
  const calculateTotal = () => {
    const subtotal = Number(orderItem?.totalPrice) || 0;
        ...
        <VStack width={{ base: '100%', md: '70%' }} spacing={4} align="stretch">
          <Flex justifyContent="space-between">
            <Text fontSize="2xl" fontWeight="bold" mb={4}>
              {!isCheckout ? 'Your Cart' : 'Checkout'} ({orderItem.items.length}{' '}
              {orderItem.items.length > 1 ? 'Items' : 'Item'})
            </Text>
            {!isCheckout && (
              <IconButton
                isRound
          ...
          {orderItem.items.map(renderProductDetails)}
        </VStack>
